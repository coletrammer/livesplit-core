...
use crate::{
    ...
    run::SegmentRange, settings::SemanticColor, timing::Snapshot,
};
...
/// TODO
pub fn best_segment_range_time(
    run: &Run,
    ...
    segment_range
        .iter()
        .map(|segment_index| run.segment(segment_index).best_segment_time()[method])
        .reduce(|acc, x| -> Option<TimeSpan> { Some(acc? + x?) })
        .flatten()
}
...
/// `run.len()`.
pub fn comparison_combined_segment_range_time(
    run: &Run,
    segment_range: &SegmentRange,
    comparison: &str,
    method: TimingMethod,
    ...
    if comparison == best_segments::NAME {
        return best_segment_range_time(run, &segment_range, method);
    }

    let current_comparison_time = run.segment(segment_range.last()).comparison(comparison)[method]?;

    let previous_comparison_time = find_previous_non_empty_comparison_time(
        &run.segments()[..segment_range.start()],
        comparison,
        method,
    )
    .unwrap_or_default();
...
// TODO
pub fn previous_segment_range_time(
    timer: &Timer,
    segment_range: &SegmentRange,
    method: TimingMethod,
) -> Option<TimeSpan> {
    segment_time(
        timer.run(),
        segment_range.start(),
        timer.run().segment(segment_range.last()).split_time()[method]?,
        method,
    )
    .into()
}
...
/// TODO
pub fn live_segment_range_time(
    timer: &Snapshot,
    segment_range: &SegmentRange,
    method: TimingMethod,
) -> Option<TimeSpan> {
...
/// TODO
pub fn previous_segment_range_delta(
    timer: &Timer,
    segment_range: &SegmentRange,
    comparison: &str,
    method: TimingMethod,
...
pub fn split_range_color(
    ...
    if show_best_segments && check_best_segment_range(timer, segment_range, method) {
        SemanticColor::BestSegment
    } else if let Some(time_difference) = time_difference.filter(|t| t != &TimeSpan::zero()) {
        let last_delta = segment_range
            .start()
            .checked_sub(1)
            .and_then(|n| last_delta(timer.run(), n, comparison, method));
        if time_difference < TimeSpan::zero() {
...
// TODO
pub fn check_best_segment_range(
    timer: &Timer,
    segment_range: &SegmentRange,
    method: TimingMethod,
) -> bool {
    if segment_range
        .iter()
        .any(|segment_index| timer.run().segment(segment_index).split_time()[method].is_none())
    {
        return false;
    }

    let delta = previous_segment_range_delta(timer, segment_range, best_segments::NAME, method);
    let current_segment = previous_segment_range_time(timer, segment_range, method);
